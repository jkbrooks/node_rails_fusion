...
var comments = require('./core/comments.js');
...
var core = module.exports = extend({}, require('./core/is.js'), require('./core/standardVars.js'), {
	reset: function() {
	...
	, '::Program': function(node) {
		// setup scopes
		var topScope = this.createTopScope(node.$scope, this.options.environments, this.options.globals, this.src.length);
		...
		var topOptions = [], from = node.range[0];

		if ( from != 0 ) {
			var newOptions = node.$scope.options.filter(function(record) {
...
//				node.$scope.add(variable.argument.name, kind, variable, variable.range[1], 0, originalDeclarator);
		...
		if (node.type === "Program") {
			...
			var newScope = node.$scope = new Scope({
				kind: "hoist",
				node: node,
				parent: null
			});

			var self = this, rangedOptions = [], minFrom = node.range[0];
			...
			node.body.body.forEach(function(method) {
				...
				var classesExtras = this.options.classesExtras || {};
				var xSpecialNode =
		...
		if ( newNotProgramScope ) {
			var from = node.range[0]
				, to = node.range[1]
			;
			var parentScope = newNotProgramScope.parent
				, parentOptions = parentScope.options
				, options = []
			;

			parentScope.options = parentOptions.filter(function(record) {
				var range = record.range, data = record.data;
	...
	, PropertyToString: function(node, justName) {
		...
		var isPrimitive = this.is.isLiteral(node) || this.is.isIdentifier(node);
	...
	unwrapRestDeclaration: function(node, donor, fromIndex) {
		...
		var sliceFunctionName = this.createVars(node, "slice");
...
//			core.setScopeTempVar(node.$scope, tempVar);
	...
	, __getNodeBegin: function(node) {
		...
		if ( node.type === "Program" ) {
			...
			var useStrictBlock$0 = !isNakedFunction && this.is.isBlock(node.body) && findUseStrictBlock(node.body);
	...
	, traverseDestructuringVariables: function(definitionNode, traverse) {
		var isObjectPattern = this.is.isObjectPattern(definitionNode);

		assert(isObjectPattern || this.is.isArrayPattern(definitionNode));

		var elementsList = isObjectPattern ? definitionNode.properties : definitionNode.elements;
	...
	, getKeyName: function(keyNode) {
		var isLiteral = this.is.isLiteral(keyNode);
	...
	, getScopeOptions: function(scope, node) {
		if ( scope === void 0 ) {
			var options$0 = this.astTopScope.options;

			return options$0['computedAll'] || (options$0['computedAll'] =
				options$0.reduce(function(computedOptions, record){
					var options = record.data;
		...
		var commentOptionsCount = this.commentOptionsCount;
		...
		var parentRange = parentScope.node.range;
		var from = parentRange[0], key = from + '-' + to;

		do {;var $break$0;(function(){
			if ( !commentOptionsCount ) {
				{$break$0 = true;return}//just fast return for files without comment options
			}
			...
			var parentComputedOptions = options;// TODO:: computed property now working not so good as expected
	...
	, detectSemicolonNecessity: function(node) {
		...
		if ( (this.is.isBlock(parent) || this.is.isProgram(parent)) && (prev = node.$previousElementSibling) ) {
			...
			;
			if ( start < end ) {
				var string = this.alter.getRange(start, end);
